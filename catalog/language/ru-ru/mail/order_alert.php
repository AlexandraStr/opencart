<?php
// Text
$_['text_subject']          = '%s - Заказ %s';
$_['text_received']         = 'Вы получили заказ.';
$_['text_order_id']         = '№ заказа:';
$_['text_date_added']       = 'Дата заказа:';
$_['text_order_status']     = 'Состояние заказа:';
$_['text_product']          = 'Товары';
$_['text_total']            = 'Итого';
$_['text_comment']          = 'Комментарий к Вашему заказу:';
$_['text_order_detail']     = 'Детализация заказа';
$_['text_instruction']      = 'Инструкции';
$_['text_payment_method']   = 'Способ оплаты:';
$_['text_shipping_method']  = 'Способ доставки:';
$_['text_email']            = 'E-mail:';
$_['text_telephone']        = 'Телефон:';
$_['text_ip']               = 'IP-адрес:';
$_['text_payment_address']  = 'Адрес плательщика';
$_['text_shipping_address'] = 'Адрес доставки';
$_['text_products']         = 'Товары';
$_['text_product']          = 'Товар';
$_['text_model']            = 'Код товара по каталогу';
$_['text_model1c']          = 'Код товара по каталогу в 1С';
$_['text_quantity']         = 'Количество';
$_['text_price']            = 'Цена';
$_['text_order_total']      = 'Заказ итого';
$_['text_total']            = 'Итого';
$_['text_customer']         = 'Данные о клиенте';
$_['text_person']           = 'Клиент';
$_['text_company']          = 'Компания(ЕДРПО,название)';
$_['text_invoice']          = 'Клиенту отправлен счет № ';

