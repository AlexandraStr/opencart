...
class ModelExtensionNews extends Model {	
    ...
    public function getTypeNews($data) {

        $sql = "SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON n.news_id = nd.news_id  LEFT JOIN " . DB_PREFIX . "news_type nt ON n.type = nt.type_id WHERE n.type ='" . (int)$data['type'] . "' AND nt.language_id = '" . (int)$this->config->get('config_language_id') . "' and nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.status = '1' ORDER BY date_added DESC";
    ...
    public function getTypeName($type) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_type  WHERE type_id = '" . (int)$type . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");

        return $query->row['type_name'];
    }

    public function getAllTypeNames() {
        $sql = "SELECT * FROM " . DB_PREFIX . "news_type WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
    ...
    public function getTypeTotalNews($type) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "news WHERE type ='" . (int)$type. "'");
