    ...
    public function setCron()
    {
        ...
        $new_cronjobs = array();
            ...
            if (isset($this->request->post['selected'])) {
                ...
                $str =($task_info['interval'] > 0) ?  "*/" .$task_info['interval'] : "*" ;
                if ($task_info['time_begin'] == $task_info['time_end']) {
                    if ($task_info['time_begin'] <> 0) {
                        $str = $str ." ". $task_info['time_begin'] . " ";
                    } else {
                        $str = $str . " *";
                    }
                } else {
                    $str = $str." ".$task_info['time_begin'] . "-" .  $task_info['time_end'];
                }
                if ($task_info['day_begin'] == $task_info['day_end']){
                    $str = $str." ".$task_info['day_begin'];
                } else {
                    $str = $str."  ". $task_info['day_begin'] . "-" .  $task_info['day_end'];
                }

                $new_cronjobs[] = $str." ".$task_info['dayofmonth']." ". $task_info ['month']. " " . __DIR__ . "/" . $task_info['task_name'] . " > /dev/null ";
            }
