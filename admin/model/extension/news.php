...
class ModelExtensionNews extends Model {
	public function addNews($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "news SET image = '" . $this->db->escape($data['image']) . "', date_added = NOW(),type = '" . (int)$data['type'] . "', status = '" . (int)$data['status'] . "'");
	...
	public function editNews($news_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "news SET image = '" . $this->db->escape($data['image']) . "',type = '" . (int)$data['type'] . "', status = '" . (int)$data['status'] . "' WHERE news_id = '" . (int)$news_id . "'");
    ...
    public function getTypeNews() {
        $sql = "SELECT * FROM " . DB_PREFIX . "news_type WHERE type_id<10 and language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getAllNews($data) {
		$sql = "SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON n.news_id = nd.news_id LEFT JOIN " . DB_PREFIX . "news_type nt ON n.type = nt.type_id WHERE nt.language_id = '" . (int)$this->config->get('config_language_id') . "' and nd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY date_added DESC";
